...
function FooterSection({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-b border-[#1C1C1E]/30 py-4 lg:border-none lg:py-0">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex w-full items-center justify-between lg:pointer-events-none"
      >
        <h3 className="font-bold text-sm lg:text-xs mb-0 lg:mb-4 text-[#C9A96E] tracking-widest uppercase">
          {title}
        </h3>
        <span className="lg:hidden text-[#C9A96E] text-xl transition-transform duration-300 transform" style={{ transform: isOpen ? "rotate(45deg)" : "rotate(0deg)" }}>
          +
        </span>
      </button>
      <div
...
export function Footer() {
  ...
  return (
    <footer className="bg-[#1C1C1E] text-[#FFFFFF] mt-0">
      ...
      <div className="max-w-[1200px] mx-auto px-4 py-8 lg:py-12 grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-8">
        ...
        <div className="mb-6 lg:mb-0">
          <div className="mb-4 flex items-center gap-2">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-xs text-[#C9A96E]/60 tracking-wide">{brand.tagline}</span>
            </div>
          </div>
          <p className="text-xs text-[#C9A96E]/60 leading-relaxed max-w-xs">
            {footer.description}
          </p>
        </div>
        ...
        <FooterSection title={footer.shopTitle}>
          <ul className="space-y-3">
            <li>
              <Link href="/collections" className="text-sm text-[#FFFFFF]/70 hover:text-[#C9A96E] transition-colors flex items-center gap-2">
                <span className="w-1 h-1 rounded-full bg-[#C9A96E]/50"></span> {footer.allProducts}
              </Link>
            </li>
            {PRODUCTS.slice(0, 4).map((p) => (
              <li key={p.slug}>
                <Link
                  ...
                  className="text-sm text-[#FFFFFF]/70 hover:text-[#C9A96E] transition-colors line-clamp-1 flex items-center gap-2"
                >
                  <span className="w-1 h-1 rounded-full bg-[#C9A96E]/50"></span> {localize(p.shortNameAr)}
                </Link>
              </li>
            ))}
          </ul>
        ...
        <FooterSection title={footer.helpTitle}>
          <ul className="space-y-3">
            {[
              ...
              <li key={href}>
                <Link href={href} className="text-sm text-[#FFFFFF]/70 hover:text-[#C9A96E] transition-colors flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[#C9A96E]/50"></span> {label}
                </Link>
              </li>
            ))}
          </ul>
        ...
        <FooterSection title={footer.trustTitle}>
          ...
          <div>
            <p className="text-xs text-[#C9A96E]/70 mb-3 font-bold">{footer.contactOn}</p>
            <div className="flex gap-3">
              <a href="https://www.tiktok.com/@baytseha" className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#C9A96E] hover:text-[#1C1C1E] text-[#C9A96E] transition-all duration-300 hover:scale-110" aria-label="TikTok">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1.04-.1z" />
                </svg>
              </a>
              <a href="#" className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#C9A96E] hover:text-[#1C1C1E] text-[#C9A96E] transition-all duration-300 hover:scale-110" aria-label="Snapchat">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
              ...
              <a href="https://www.instagram.com/baytseha/" className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#C9A96E] hover:text-[#1C1C1E] text-[#C9A96E] transition-all duration-300 hover:scale-110" aria-label="Instagram">
              ...
              <a href="#" className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#C9A96E] hover:text-[#1C1C1E] text-[#C9A96E] transition-all duration-300 hover:scale-110" aria-label="YouTube">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="border-t border-[#1C1C1E]/40 py-6 px-4 bg-[#1C1C1E]">
        <div className="max-w-[1200px] mx-auto flex flex-col items-center">
          <p className="text-xs text-[#C9A96E]/30 text-center">
