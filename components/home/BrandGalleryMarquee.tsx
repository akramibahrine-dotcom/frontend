...
export function BrandGalleryMarquee() {
  ...
  return (
    <section
      className="py-14 md:py-20 bg-[#1C1C1E] overflow-hidden border-y border-[#1C1C1E]/40"
    ...
    >
      <div className="max-w-[1200px] mx-auto px-4 mb-8 md:mb-10 text-center">
        ...
        <p className="text-[#C9A96E]/80 text-sm md:text-base max-w-xl mx-auto">
      ...
      <div className="relative md:hidden px-2">
        <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 px-2 [-webkit-overflow-scrolling:touch]">
          {GALLERY_ITEMS.map((item) => (
            <figure
              key={item.id}
              className="snap-center shrink-0 w-[75vw] max-w-xs rounded-3xl overflow-hidden border border-[#C9A96E]/25 shadow-lg"
            >
              ...
              <img
                ...
                loading="lazy"
              />
              <figcaption className="bg-[#1C1C1E] text-center py-3 px-2">
                <span className="text-[#C9A96E] font-bold text-sm">{item.nameAr}</span>
              </figcaption>
            </figure>
          ))}
        </div>
      ...
      <div className="hidden md:block relative" dir="ltr">
        <div className="pointer-events-none absolute inset-y-0 left-0 w-20 z-10 bg-gradient-to-r from-[#1C1C1E] to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-20 z-10 bg-gradient-to-l from-[#1C1C1E] to-transparent" />

        <div className="flex gap-6 lg:gap-8 animate-marquee-brand hover:[animation-play-state:paused] py-2 items-stretch">
          {loop.map((item, i) => (
            <figure
              key={`${item.id}-${i}`}
              className="shrink-0 w-[min(38vw,380px)] lg:w-[400px] rounded-[2rem] overflow-hidden border border-[#C9A96E]/20 shadow-xl group"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={item.image}
